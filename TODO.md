...
14. **Release `v1.0.0` and the container package** — PREPARED on `release-v1`: version 1.0.0 (single source in `pyproject.toml`; settings read it), `CHANGELOG.md`, OCI labels in the Dockerfile, `manage.py export_directory`, and the tag-triggered `publish` (tag/version check, amd64+arm64, `v1.0.0` + `latest`) and `release` (assets + notes from the changelog) jobs, linted with actionlint and dry-run locally. **Remaining (public, ask first):** PR `release-v1` → `main` and merge; tag `v1.0.0` on `main`; watch the run; the user makes the GHCR package public; then add the release and container badges to the README. If the release date moves, update the date in `CHANGELOG.md` first.
