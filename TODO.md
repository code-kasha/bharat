...
- **Tasks 1–14 are done: v1.0.0 is released** and the container image is public. Next: task 15 (demo), then 17 and 16.
...
15. **Deploy the demo on a free tier, with a planned end date** — IN PROGRESS. Decided: **Render** (no card, 750 h/month, sleeps after 15 min; Cloud Run needs a billing card and Hugging Face Docker Spaces need a paid plan, per 2026 search results; the provider sites are blocked in this cloud environment), end date **26 December 2026**. Built: `SITE_DEMO_UNTIL` notice on every page, `/health/` reports `version` and `revision`, the image listens on `$PORT`, and a CI `deploy` job (`production` environment; skipped until `vars.DEMO_URL` is set) that triggers `secrets.DEPLOY_HOOK_URL` and waits for the new revision. Steps in `docs/deployment.md` "Hosted demo". Live at https://bharat-post-dir.onrender.com/ (created by the user, first built from `main` before PR code-kasha/bharat-post-dir#3 merged; `onrender.com` is blocked in this cloud environment, so verify it from a browser). README demo line added. **Remaining:** merge #3 so the next deploy carries the demo notice and `/health/` revision; confirm the `deploy` job (needs `vars.DEMO_URL` and `secrets.DEPLOY_HOOK_URL`); the user sets the repository website to the demo URL; set the shutdown reminder (task 16).
